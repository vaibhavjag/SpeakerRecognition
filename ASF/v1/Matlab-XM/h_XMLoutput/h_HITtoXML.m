function fileName=...
   HITtoXML(LogAttackTime, HarmonicSpectralCentroid, HarmonicSpectralDeviation, HarmonicSpectralSpread, HarmonicSpectralVariation, fileName)
...
defaultFileName='HarmonicInstrumentTimbre.xml';
...
tmp(2).tag = 'HarmonicSpectralCentroid';
tmp(2).value = num2str(HarmonicSpectralCentroid);
tmp(2).type = 'scalar';

tmp(3).tag = 'HarmonicSpectralDeviation';
tmp(3).value = num2str(HarmonicSpectralDeviation);
tmp(3).type = 'scalar';

tmp(4).tag = 'HarmonicSpectralSpread';
tmp(4).value = num2str(HarmonicSpectralSpread);
tmp(4).type = 'scalar';

tmp(5).tag = 'HarmonicSpectralVariation';
tmp(5).value = num2str(HarmonicSpectralVariation);
tmp(5).type = 'scalar';


%Descriptor generation:
%line='<AudioDescriptionScheme xsi:type="mpeg7:HarmonicInstrumentTimbreType">';
line='<DescriptionUnit xsi:type="mpeg7:HarmonicInstrumentTimbreType">';
fprintf(xmlFile,'\t%s\n',line);
