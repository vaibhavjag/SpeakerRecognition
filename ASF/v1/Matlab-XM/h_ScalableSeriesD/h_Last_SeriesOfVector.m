...
lastValues = zeros(sumElement, vectorSize);
...
if len == 1 & scalingRatio(len) == 1& write_flag == 1
   ...
   k = 1;
         ...
         for i = 1:elementNum(k)
            ...
            if columnNum == 0
              for j = 1:vectorSize
               ...
               len1 = length(sn_segment);  
               lastValues(i,j) = sn_segment(len1);
              end
	...
	[rows, columns] = size(lastValues);
	x = strcat('	<Last dim = "', num2str(rows), '	', num2str(columns),'">');
	fprintf(fid, '%s ',x);
	for i = 1:rows-1
		x = strcat(num2str(lastValues(i, 1:vectorSize)));
   	fprintf(fid, '	%s \n',x);
	end
	x = strcat(num2str(lastValues(rows, 1:vectorSize)));
	x = strcat(x, '</Last>');
	fprintf(fid, '	%s \n',x);
