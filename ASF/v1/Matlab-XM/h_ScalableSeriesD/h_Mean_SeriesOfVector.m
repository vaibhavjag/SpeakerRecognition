...
meanValues = zeros(sumElement, vectorSize);
...
if weight_flag == 0
   if len == 1 & scalingRatio(len) == 1& write_flag == 1
         ...
         for i = 1:elementNum(k)
            ...
            if columnNum == 0
              for j = 1:vectorSize
               ...
               meanValues(i,j) = mean(sn_segment);
              end
      ...
      [rows, columns] = size(meanValues);
      x = strcat('	<Mean dim = "', num2str(rows), '	', num2str(columns),'">');
      fprintf(fid, '%s ',x);
      for i = 1:rows-1
      	x = strcat(num2str(meanValues(i, 1:vectorSize)));
         fprintf(fid, '	%s \n',x);
      end
      x = strcat(num2str(meanValues(rows, 1:vectorSize)));
      x = strcat(x, '</Mean>');
      fprintf(fid, '	%s \n',x);
	end
else
               ...
               if columnNum == 0 & columnNumOld == 0
                  ...
                  for j = 1:vectorSize
                     ...
                     for m = 1:elementNum(k)
               		...
               		meanValues(i,j) = mean(sn_seg(1:l-1));
                  end
