...
minValues = zeros(sumElement, vectorSize);
...
if weight_flag == 0
   if len == 1 & scalingRatio(len) == 1& write_flag == 1
      ...
      [rows, columns] = size(minValues);
      x = strcat('	<Min dim = "', num2str(rows), '	', num2str(columns),'">');
      fprintf(fid, '%s ',x);
      for i = 1:rows-1
      	x = strcat(num2str(minValues(i, 1:vectorSize)));
         fprintf(fid, '	%s \n',x);
      end
      x = strcat(num2str(minValues(rows, 1:vectorSize)));
      x = strcat(x, '</Min>');
      fprintf(fid, '	%s \n',x);
	end
else
               ...
               if columnNum == 0 & columnNumOld == 0
                  ...
                  for j = 1:vectorSize
                     ...
                     for m = 1:elementNum(k)
               		...
               		minValues(i,j) = min(sn_seg(1:l-1));
                  end
